        ...
        foreach ($users as $user) {
            switch ($user->type) {
                ...
                case "Tournant2":
                    ...
                    break;
                case "Tournant3":
                    $this->generateTournant3Horaires($user);
                    break;
            }
    ...
    protected function generateFixeHoraires(User $user)
    {
        ...
        while ($date->year == now()->year) {
            ...
            if ($date->dayOfWeek !== 5 && $date->dayOfWeek !== 6) {
                ...
                $this->error("Creation d'une horaire pour <fg=red>{$user->nom}</> de <fg=green>{$date->format('Y-m-d H:i:s')} a {$date->copy()->setTime(16, 0, 0)->format('Y-m-d H:i:s')}</>");
            }
            $date = $date->addDays(1);
        }
    }
